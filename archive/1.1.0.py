...
def handle_event(event):
    global selected, whichnew, ideas,exploremode, viewday, input, mode, start, colors, anychange, ideacat, dhstop, dhstart, zoom, hstart, hstop, eventl, otherday, savelist, hline, cati, m, dlist, remind, smslist, messageinfo, remindinfo, annivs, annivsinfo, base, ideabox, missedcalls, unreadsms,weekday, menu, alarm, screenObj, onstandby
    ev = event['keycode']
    pi = event['scancode']
...
def prepare(datetime):
    ...
    for k in d:
        ...
        input.extend([[start[3]-start[-1],start[4],end[3]-end[-1],end[4],z.content,z.location,z.priority,z.id]])
    if input==[]:
        input=[[]]
